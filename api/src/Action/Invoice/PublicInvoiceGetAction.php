...
final class PublicInvoiceGetAction
{
    ...
    public function __invoke(Request $request, Response $response, array $args): Response
    {
        ...
        // Nový upstream public_token má přednost. Starší vlastní odkazy používají
        // hashovaný public_view_token; fallback zachová jejich platnost po upgradu.
        $invoice = $this->repo->findByPublicToken($token)
            ?? $this->repo->findByPublicViewToken($token); // drafty filtrují oba SQL dotazy
        if ($invoice === null) {
        ...
        if (!is_array($user) || empty($user['id'])) {
            ...
            $this->repo->markPublicLinkOpened((int) $invoice['id']);
            if ($firstView) {
            ...
            $this->logger->log('invoice.public_link_opened', null, 'invoice', (int) $invoice['id'], [
