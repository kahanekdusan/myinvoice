...
const newDocPath = computed(() => (quoteMode.value ? '/price-quotes/new' : '/invoices/new'))
const pageTitle = computed(() => (quoteMode.value ? 'Cenové nabídky' : t('invoice.title')))
const pageSubtitle = computed(() => (quoteMode.value ? 'Přehled cenových nabídek.' : t('invoice.subtitle_grouping')))
const newButtonLabel = computed(() => (quoteMode.value ? '+ Nová cenová nabídka' : t('invoice.new')))

useHotkey('ctrl+n', (e) => { e.preventDefault(); router.push(newDocPath.value) })
...
function syncFiltersToUrl() {
  ...
  if (!quoteMode.value && typeFilter.value) q.type = typeFilter.value
  ...
  <div>
    <div class="flex items-center justify-between mb-4 gap-3 flex-wrap">
      <div>
        ...
        <p class="text-sm text-neutral-500 mt-0.5">{{ pageSubtitle }}</p>
      </div>
    ...
    <div class="bg-white border border-neutral-200 rounded-lg shadow-sm mb-4 p-3">
      <div class="flex flex-wrap items-center gap-2">
        ...
        <select v-if="!quoteMode" v-model="typeFilter" class="h-9 px-3 border border-neutral-300 rounded-md bg-white text-sm">
